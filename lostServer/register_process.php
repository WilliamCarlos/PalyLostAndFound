...
if(count($_POST)>0) {
    if (!isset($_POST['userName']) ||
        ...
        header("Location: http://lostandfound.paly.net/lost/register.php?errMsg=" . $errMsg);
        return;
    }                    
    ...
    $pos = strpos($userId, "@pausd.us");
    if ($pos === false) {
        $errMsg = "Not a valid @pausd.us email address";
        header("Location: http://lostandfound.paly.net/lost/register.php?errMsg=" . $errMsg);
        return;
    }
    ...
    $verifyUrl = "http://lostandfound.paly.net/lost/email_verify.php?random=" . $rnd;
    sendWelcomeEmail($userId, $firstName, $verifyUrl);

    header("Location: http://lostandfound.paly.net/lost/registerSuccess.php");
}
?>