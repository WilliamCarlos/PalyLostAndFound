...
<header>
  <link rel="shortcut icon" href="/lost/favicon.ico", type="image/x-icon">
  ...
  <nav class="top-bar">
	...
	<li class="divider"></li>
	<li><a href="/lost/claimed.php">Claimed Items</a></li>
	<li class="divider"></li>
...
function echoTopImg($imgName) {
  echo "<div><center>";
  // echo "<div class='logoPos'><img src='/lost/img/PalyLostFoundLogoBig.jpg' data-action='zoom'></div>";
  echo "<div class='logoPos'><a href='/lost/index.php'><img src='/lost/img/PalyLostFoundLogo.jpg'></a></div>";
  echo "<div class='palyLogoPos'><a href='/lost/index.php'><img src='/lost/img/PalyLogo.png'></a></div>";
  echo "<dir class='pageTitle'><img src='/lost/img/" . $imgName . "' width='900px'></td></div>";
  echo getWelcomeMsg();
  echo "</div>";
}
...
function echoMorePages($conn, $dataQry, $hrefPage, $currPage, $pageLength) {
    ...
    for ($idx=1; $idx<=$totalPages; $idx++) {
       if($idx == $currPage) {
          $pageHyperLink = "<i><b><u>$idx</b></u></i>";
       } else {
          $pageHyperLink = "<u>$idx</u>";
       }
...
function sendPasswdEmail($userId, $firstName, $verifyUrl) {
    ...
    $headers = "From: register@palylostandfound.com\r\nReply-To: no-reply@palylostandfound.com\r\nX-Mailer: PHP/" . phpversion();
    mail($userId, $subject, $message, $header);
}

function isAdminstrator($userId) {
    $conn = mySqlConn();
